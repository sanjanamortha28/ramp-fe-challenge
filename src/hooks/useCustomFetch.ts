...
export function useCustomFetch() {
  ...
  //const endpoint: RegisteredEndpoints = "employees";
  ...
  const fetchWithCache = useCallback(
    async <TData, TParams extends object = object>(
      ...
      // If endpoint is "allTransactions", fetch all transactions directly without caching
      if (endpoint === "allTransactions") {
        const allTransactionsResponse = await fetch('url_to_fetch_all_transactions');
        return allTransactionsResponse.json() as Promise<TData>;
      }
  
      // For other endpoints, use caching mechanism as usual
